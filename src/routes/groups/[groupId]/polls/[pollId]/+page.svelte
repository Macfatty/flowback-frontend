	...
	{#if poll}
		...
		{#if pollType === 4}
			...
			{:else if phase === 'proposal'}
				<Structure bind:phase bind:poll>
					<div slot="left" class="h-[450px] relative">
					...
					<div slot="right" class="relative h-[450px]">
						{#if selectedProposal}
			...
			{:else if phase === 'prediction_statement'}
				<Structure bind:phase bind:poll>
					...
					<div slot="right" class="relative h-[450px] overflow-hidden">
						{#if selectedProposal}
							<div class="flex flex-col space-y-2 p-2">
			...
			{:else if phase === 'vote'}
				<Structure bind:phase bind:poll>
					<div slot="left" class="h-[450px]" id="proposals-section">
						<span class="text-xl font-semibold mb-4 ml-3 text-primary dark:text-secondary"
							>{$_('Proposals')} ({proposals?.length})</span
						>
			...
			{:else if phase === 'result' || phase === 'prediction_vote'}
				<Structure bind:phase bind:poll>
					<div slot="left" class="h-[450px] overflow-y-auto">
						{#if proposals}
